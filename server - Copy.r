...
#data <- read.csv("monthly_Data.csv")
...
#     query <-"INSERT INTO monthly_data 
...
# %d,input$numbers);"
    ...
    datemonthly <- as.Date(Sys.Date())

    dbGetQuery(con
               ,paste0("Insert monthly_data 
                        values ('",input$first,"','",input$last,"','",input$stores1,"','",input$numbers,"','",input$quantity,"','",input$description,"',
                       '",datemonthly,"','","Pending","')"))
    ...
    updateSelectInput(session,"stores1","Select A Store", c("","Hamidas Touch", "The Pinepple"))
...
################################################################################################################################################################################
  ...
  observeEvent(input$refresh,{  output$table <- DT::renderDataTable({
    ...
    rs <- dbSendQuery(con, "SELECT * FROM monthly_data")
    data <- dbFetch(rs)
  ...
  output$x4 = renderPrint({
    ...
    t = data[s, 1]
    if (length(t)) {
      cat('Please Enter The Phone Number For:')
      cat(t, sep = ', ')
    }
  })
...
#################################################################################################################################################################################  
  ...
  ####################################ALWOSI DATA#######################################################################################
  output$table_alwosi <- DT::renderDataTable({
    con <- DBI::dbConnect(odbc::odbc(),
                          driver = "MySQL ODBC 8.0 Unicode Driver",
                          database = "test_db",
                          UID    = "root",
    ...
    rs <- dbSendQuery(con, "SELECT * FROM alwosi_data")
    data <- dbFetch(rs)
    data <- filter(data, data$Item == input$alwosi_items) 
